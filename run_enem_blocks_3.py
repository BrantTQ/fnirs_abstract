...
# ======== CONFIG (paths) ========
# Put logs next to this script (stable even if you launch from another folder)
...
QUESTIONS_PER_BLOCK = 6        # Questions per block
CONCRETE_PER_BLOCK = 3         # Concrete questions per block
ABSTRACT_PER_BLOCK = 3         # Abstract questions per block
...
try:
    ...
    print(f"[LOG] Could not open log file at {log_path}: {e}")
    # show_message uses visual objects; if window failed above we would have exited.
    # Here it's safe to show a message, but if it fails we quit.
    try:
...
log_f.flush()  # ensure header hits disk
...
def log_event(phase, block_label, trial_idx, q_data, marker_name, code, t_phase_start,
    ...
    # Absolute time and reaction time
    t_abs = global_clock.getTime()
    rt = (t_abs - t_phase_start) if t_phase_start is not None else ""

    # Safe question metadata extraction
    if isinstance(q_data, dict):
        q_num   = q_data.get("question_number", "")
        q_year  = q_data.get("year", "")
        q_type  = q_data.get("type", "")
        q_field = q_data.get("field", "")
    else:
        q_num = q_year = q_type = q_field = ""

    # Write the row exactly matching your header
    log_writer.writerow([
        f"{t_abs:.6f}",            # t_abs
        phase,                     # phase
        block_label,               # block
        trial_idx,                 # trial_idx_in_block
        q_num,                     # question_number
        q_year,                    # question_year
        q_type,                    # question_type
        q_field,                   # question_field
        marker_name,               # marker_name
        code,                      # marker_code
        f"{rt}",                   # rt_from_phase
        choice,                    # choice
        correct,                   # correct
        button_click_t,            # button_click_time
        opt_view_t,                # option_view_time
        note                       # note
    ])
    log_f.flush()
...
def run_trial(block_label, idx_in_block, question_data):
    """Run a single trial with two-phase presentation, allowing ONE back-to-text."""
    ...
    # One-time back-to-text controls (local UI elements)
    back_btn = visual.Rect(win, width=360, height=60, fillColor=[-0.2, -0.2, -0.2],
                           lineColor="white", pos=(0, -300))
    back_lbl = visual.TextStim(win, text="Back to text (once)  [B]", color="white",
                               height=GEN_TEXT_HEIGHT, pos=(0, -300))
    ...
    while chosen is None:
        # Draw full question + options
        question_itself.draw()
        for i in range(5):
            opt_texts[i].draw()
            opt_boxes[i].draw()

        # Draw back button only if not yet used
        if not back_used:
            back_btn.draw()
            back_lbl.draw()
        ...
        if mouse.getPressed()[0]:
            ...
            for i, box in enumerate(opt_boxes):
                if box.contains(mouse):
                    ...
                    break
            # Back button?
            if chosen is None and (not back_used) and back_btn.contains(mouse):
                back_used = True
                log_event("back_to_text", block_label, idx_in_block, question_data,
                          "BACK_TO_TEXT", 0, full_on, note="participant requested back once")

                # Go to text again
                mouse.clickReset()
                kb.clearEvents()
                question_text.text = question_data["question_text_translated"]
                text_on2 = global_clock.getTime()
                mname, mcode, _ = send_marker("Q_TEXT_ON")
                log_event("question_text", block_label, idx_in_block, question_data, mname, mcode, text_on2,
                          note="revisit=1")
                ...
                while not re_show:
                    ...
                    if mouse.getPressed()[0] and button_show.contains(mouse):
                        button_click_time2 = global_clock.getTime()
                        mname, mcode, _ = send_marker("BUTTON_CLICK")
                        re_show = True

                    keys2 = kb.getKeys(['space', 'escape'], waitRelease=False)
                    if keys2:
                        if keys2[0].name == 'escape':
                            cleanup_and_quit()
                        button_click_time2 = global_clock.getTime()
                        mname, mcode, _ = send_marker("BUTTON_CLICK")
                        re_show = True
                ...
                # Re-enter full view
                full_on = global_clock.getTime()  # reset baseline for RT
                mname, mcode, _ = send_marker("Q_FULL_ON")
                log_event("question_full", block_label, idx_in_block, question_data, mname, mcode, full_on,
                          button_click_t=f"{button_click_time2:.6f}", note="revisit=1")

                continue  # redraw full view

        if chosen is not None:
            break

        # --- Keyboard handling ---
        keys = kb.getKeys(['a','b','c','d','e','1','2','3','4','5','escape','b'], waitRelease=True)
        if keys:
            name = keys[0].name
            if name == 'escape':
                cleanup_and_quit()
                ...
                chosen = chr(64 + int(name))  # 1->A etc.
            elif name == 'b' and (not back_used):
                back_used = True
                log_event("back_to_text", block_label, idx_in_block, question_data,
                          "BACK_TO_TEXT", 0, full_on, note="participant requested back once (key)")
                ...
                full_on = global_clock.getTime()  # reset for RT after revisit
                mname, mcode, _ = send_marker("Q_FULL_ON")
                log_event("question_full", block_label, idx_in_block, question_data, mname, mcode, full_on,
                          button_click_t=f"{button_click_time2:.6f}", note="revisit=1")

                # continue to allow answering
